...
public class ReplyArticleService {
	...
	private void checkParent(Article parent, int parentId) throws ArticleNotFoundException, CannotReplyArticleException {
		...
		if(parent == null) {
			throw new ArticleNotFoundException("not parent article" + parentId);
		}
		int parentLevel = parent.getLevel();
		if(parentLevel == 3) {
			throw new CannotReplyArticleException("마지막 라벨 글에는 답글 달 수 없음" + parent.getId());
		}
	}
	...
	private String getSearchMinSeqNum(Article parent) {
		...
		case 0:
			searchMinLongValue = parentSeqLongValue / 1000000L * 1000000L;   //뒤에 6자리 0으로 만듬
			break;
		case 1:
			searchMinLongValue = parentSeqLongValue / 10000L * 10000L;       //뒤에 4자리 0으로 만듬
			break;
		case 2:
